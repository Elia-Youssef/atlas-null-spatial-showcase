...
export function Reveal({
  ...
  return (
    <motion.div
      ...
      // Reduced-motion preference is unknown during SSR. A stable initial
      // transform prevents hydration drift; the zero-duration client
      // transition removes it immediately when reduced motion is requested.
      initial={{ opacity: 1, ...DIRECTION_OFFSET[direction] }}
      onAnimationComplete={() => {
  ...
  return (
    ...
    >
      {Children.map(children, (child) => (
        <motion.div
          variants={{
            // Keep the server and first client render identical. Reduced
            // motion controls timing rather than changing the DOM snapshot.
